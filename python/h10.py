...
def min_red_deg_vertex(g, q):
    (black_edges, red_edges) = g
    rd, v = None, None
    while(True):
        rd, v = q.get()
        if v in black_edges and len(red_edges[v]) == rd:
            break
        else:
            continue
    min_red_deg_v = v
...
def calculate_rd(g, x, y):
    (black_edges, red_edges) = g
    black_edges, red_edges = g
    z = black_edges[x] ^ black_edges[y] | red_edges[x] | red_edges[y]
    z -= {x, y}
    return len(z)
...
def contract(g, e, q):
    ...
    black_edges.pop(w)
    red_edges.pop(w)
    for x in black_edges[v].union(red_edges[v]):
    
        q.put((len(red_edges[x]), x))
...
def contracting_vertices(g, q):

    
    (black_edges, red_edges) = g
    
    x = min_red_deg_vertex(g, q)
    y = None
    min_rd = float('inf')
